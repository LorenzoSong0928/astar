...
# 2. 生成合法邻居
def get_directional_neighbours(state, grid, clearance=0):
...
def is_available(x, y, grid, clearance=0):
    height = len(grid)
    width = len(grid[0])
...
print(
    "clearance=0：",
    is_available(test_pos[0], test_pos[1], grid, 0)
)

print(
    "clearance=1：",
    is_available(test_pos[0], test_pos[1], grid, 1)
)

for clearance in [0, 1, 2]:
